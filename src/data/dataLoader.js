...
export const DATA_CACHE_VERSION = 'move-picker-embedded-learnset-fallback-2026-05-25';
...
function runtimeRootAssetUrl(path) {
...
export async function loadMoveLegalityData(data = activeDataRef) {
  ...
  try {
    ...
    console.warn('Move learnset split file failed, trying bundled db fallback:', splitError);
    try {
      payload = await loadBundledMoveLegalityFallback();
      rows = Array.isArray(payload?.pokemon_moves) ? payload.pokemon_moves : Array.isArray(payload) ? payload : [];
    } catch (fallbackError) {
      console.warn('Move learnset bundled fallback failed, deriving from embedded Pokémon move references:', fallbackError);
...
async function loadBundledMoveLegalityFallback() {
  const attempts = [
    runtimeAssetUrl('pokemon_moves.json'),
    runtimeRootAssetUrl('db.json'),
